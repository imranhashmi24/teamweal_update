            ...
            <div class="row">
                        ...
                        <div class="input-group">
                            <select class="form-control" name="type">
                                ...
                                @foreach ($eventTypeElements as $type)
                                    <option value="{{ $type->data_values->title }}"
                                        {{ old('type') == $type->data_values->title ? 'selected' : '' }}
                                    >
                                        @if(app()->getLocale() == 'en')
                            ...
                            <select class="form-control" name="audience_type">
                                ...
                                @foreach ($audienceTypeElements as $audienceType)
                                    <option value="{{ $audienceType->data_values->title }}"
                                        {{ old('audience_type') == $audienceType->data_values->title ? 'selected' : '' }}
                                    >
                                        @if(app()->getLocale() == 'en')
                            ...
                            <select class="form-control" name="sector">
                                ...
                                @foreach ($eventSectorElements as $eventSector)
                                    <option value="{{ $eventSector->data_values->title }}"
                                        {{ old('sector') == $eventSector->data_values->title ? 'selected' : '' }}
                                    >
                                        @if(app()->getLocale() == 'en')
                ...
                <div class="my-3 col-12 col-md-12 col-lg-12">
                    <div class="form-group d-flex justify-content-start">
                        ...
                        <div class="input-group">
                            <input type="checkbox" name="same_time" value="1" {{ old('same_time') == 1 ? 'checked' : '' }} class="custom-checkbox" id="sameTimeCheckbox">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-3 col-12 col-md-6 col-lg-6" id="eventDateField" style="display: none;">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="datetime-local" name="same_time_date" value="{{ old('same_time_date') }}" class="form-control">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="mb-3 col-12 col-md-6 col-lg-6">
                    <div class="form-group">
                        ...
                        <select name="country_id" class="form-control" required>
                            ...
                            <option value="{{ $country->id }}" data-cities="{{ $country->city }}" @selected(old('country_id'==@$country->id))>
                                @if(app()->getLocale() == 'en')
                                {{ $country->name }}
                                @else
                                {{ $country->name_ar }}
...
<script>
    ...
    $('[name=country_id]').on('change', function() {
        ...
        $.each(cities, function(index, value) {
            ...
            option += "<option value='" + value.id + "' " + (value.id == "{{ old('city_id') }}" ? "selected" : "") + "data-lat='" + value.lat + "' data-lng='" + value.lng + "'>" +
                name + "</option>";
        });
